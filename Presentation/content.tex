    ...
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            ...
            \begin{itemize}
                ...
                \item аукционный распределитель с оптимизацией $SW$;
...
\begin{frame}
	...
	\begin{tabular}{p{0.12\linewidth}p{0.49\linewidth}p{0.28\linewidth}}
		...
		\midrule
		H1 & польза аукционного $SW$-распределения при фиксированных режимах & подтверждена \\
		H2 & польза обучаемого выбора режимов & подтверждена \\
		H3 & выигрыш полной гибридизации над одиночными компонентами & подтверждена \\
		H4 & диагностическая информативность внутренних цен & частично подтверждена \\
		H5 & устойчивость к пиковой нагрузке & частично подтверждена \\
		H6 & устойчивость к деградации связи и отказам & частично подтверждена \\
		H7 & масштабируемость C-AMPLE-AMR & не подтверждена \\
		\bottomrule
	\end{tabular}
	\medskip
	
	\begin{block}{Интерпретация}
		Обе составляющие гибридного метода вносят вклад в $SW$: аукционный слой оптимизирует ценность выбранного подмножества задач, а QMIX адаптирует условия распределения. При этом возникает содержательный trade-off: эвристика чаще даёт лучший throughput, а аукционные методы выигрывают по общественному благосостоянию.
	\end{block}
\end{frame}

\begin{frame}
	\frametitle{Общие результаты валидации}
	\small
	\begin{itemize}
		\item Fixed-Auction превосходит Fixed-Heuristic по $SW$ во всех 70 сопоставлениях; средний эффект составляет $+558.7$.
		\item QMIX-Heuristic также улучшает $SW$ относительно Fixed-Heuristic; средний выигрыш составляет $+82.2$.
		\item AMPLE-AMR в среднем превосходит одно-компонентные варианты на $+326.1$, но в разнородной инфраструктуре уступает Fixed-Auction.
		\item C-AMPLE-AMR оправдан прежде всего на Warehouse-XL, где overhead снижается с $264.1$ до $51.3$~мс при потере welfare $41.9$.
	\end{itemize}
\end{frame}
